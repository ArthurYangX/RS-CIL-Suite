"""ER-ACE — Asymmetric Cross-Entropy replay (Caccia et al., ICLR 2022).

Like ER, but uses asymmetric CE:
  - NEW task samples: CE only over new-class logits (mask old classes)
  - REPLAY samples: CE over all seen-class logits (standard)
This prevents new-task gradients from pushing old-class logits down.
"""
...
class ERACE(CILMethod):
    ...
    def train_task(self, task: Task, train_loader: DataLoader):
        ...
        cids = sorted(self.seen_classes)              # all seen class ids
        new_cids = sorted(task.global_class_ids)      # current task class ids
        g2l_all = {g: i for i, g in enumerate(cids)}  # global -> local (all seen)

        # Local indices of new classes within cids
        new_local = [g2l_all[g] for g in new_cids]
        ...
        for ep in range(self.epochs):
            ...
            for xh, xl, y in train_loader:
                ...
                # Asymmetric CE for new samples: only use new-class logits
                new_logits = logits[:, new_cids]
                g2l_new = {g: i for i, g in enumerate(new_cids)}
                mapped_new = torch.tensor([g2l_new[yi.item()] for yi in y],
                                          device=self.device)
                loss = F.cross_entropy(new_logits, mapped_new)

                # Standard CE for replay samples: use all seen-class logits
                buf = self._sample_buffer(min(len(y), 64))
                if buf is not None:
                    bh, bl, by = [t.to(self.device) for t in buf]
                    b_logits = self.head(self.model(bh, bl))
                    bmapped = torch.tensor([g2l_all.get(yi.item(), 0) for yi in by],
                                           device=self.device)
                    valid = torch.tensor([yi.item() in g2l_all for yi in by],
                                         device=self.device)
                    if valid.any():
